...
const pindahanSchema = z.object({
  ...
  // Dua field tahun — inti dari modul pindahan
  tahun_masuk: z.coerce
    .number()
    .min(2000, 'Tahun tidak valid')
    .max(currentYear, 'Tahun masuk tidak boleh di masa depan'),
  tahun_angkatan: z.coerce
    .number()
    .min(2000, 'Tahun tidak valid')
    .max(currentYear, 'Tahun angkatan tidak boleh di masa depan'),
...
type PindahanFormValues = z.infer<typeof pindahanSchema>;
...
export default function PindahanSiswaPage() {
  const router = useRouter();
  const [currentStep, setCurrentStep] = useState(0);
  const [isSubmitting, setIsSubmitting] = useState(false);

  const methods = useForm<PindahanFormValues>({
    resolver: zodResolver(pindahanSchema),
    defaultValues: {
      status: ['Umum'],
      guardian_type: 'ayah',
      tahun_masuk: currentYear,
      tahun_angkatan: currentYear - 1,
    },
    mode: 'onTouched',
  });
  ...
  const tahunMasuk = watch('tahun_masuk');
  const tahunAngkatan = watch('tahun_angkatan');

  // Autosave — key berbeda dari form tambah biasa
  const { saveNow, clear, restore } = useAutoSave<PindahanFormValues>('form_pindahan_siswa', watch(), true);
  ...
  const nextStep = async () => {
    ...
    if (currentStep === 0) {
      fieldsToValidate = [
        'name', 'nisn', 'nik', 'gender', 'birth_place', 'birth_date',
        'tahun_masuk', 'tahun_angkatan', 'guardian_type',
      ];
  ...
  const onSubmit = async (data: PindahanFormValues) => {
    setIsSubmitting(true);
    try {
      const res = await api.post('/students', data);
      clear();
      toast.success('Siswa pindahan berhasil ditambahkan!');
      ...
      <div className="mb-6 flex items-center gap-4">
        ...
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-serif font-bold text-slate-800">Tambah Siswa Pindahan</h1>
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-amber-100 text-amber-700 border border-amber-200">
              <AlertTriangle className="w-3 h-3" />
              Pindahan
            </span>
          </div>
          <p className="text-stone-500 mt-1">Masukkan data buku induk siswa yang masuk melalui jalur pindahan</p>
        </div>
      </div>

      {/* Info banner perbedaan tahun */}
      {currentStep === 0 && (
        <div className="max-w-4xl mx-auto mb-6 p-4 rounded-xl border border-amber-200 bg-amber-50 flex gap-3">
          <AlertTriangle className="w-5 h-5 text-amber-500 flex-shrink-0 mt-0.5" />
          <div className="text-sm text-amber-800">
            <p className="font-semibold mb-1">Perhatian: Dua Field Tahun Berbeda</p>
            <ul className="space-y-1 list-disc list-inside text-amber-700">
              <li><strong>Tahun Masuk</strong> — tahun siswa bergabung ke sekolah <em>ini</em> (contoh: 2025, masuk kelas 11)</li>
              <li><strong>Tahun Angkatan</strong> — tahun angkatan asli siswa di sekolah asal (contoh: 2024, angkatan kelas 10)</li>
            </ul>
            {tahunMasuk && tahunAngkatan && tahunMasuk === tahunAngkatan && (
              <p className="mt-2 text-amber-600 font-medium">
                ⚠️ Saat ini kedua tahun bernilai sama ({tahunMasuk}). Pastikan ini benar jika siswa memang pindah di kelas 10.
              </p>
            )}
          </div>
        </div>
            ...
            <CardFooter className="justify-between">
              ...
              {currentStep < steps.length - 1 ? (
                ...
                <Button type="button" variant="primary" isLoading={isSubmitting} onClick={handleSubmit(onSubmit)}>
                  <Save className="w-4 h-4 mr-2" /> Simpan Data Pindahan
                </Button>
              )}
            </CardFooter>
          </form>
...
// ── Step 1: Identitas ── (dengan dua field tahun)
function StepIdentitas() {
  const { register, formState: { errors }, watch } = useFormContext<PindahanFormValues>();
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <CardTitle>Identitas Siswa Pindahan</CardTitle>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        {/* ── Dua Field Kunci untuk Pindahan ── */}
        <div className="md:col-span-2 p-4 rounded-xl border border-blue-100 bg-blue-50">
          <p className="text-xs font-semibold text-blue-700 uppercase tracking-wide mb-3">
            Informasi Tahun (Khusus Pindahan)
          </p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <Input
              label="Tahun Masuk (ke sekolah ini)"
              type="number"
              {...register('tahun_masuk')}
              error={errors.tahun_masuk?.message}
              hint="Tahun siswa bergabung ke sekolah ini"
              required
            />
            <Input
              label="Tahun Angkatan (asal)"
              type="number"
              {...register('tahun_angkatan')}
              error={errors.tahun_angkatan?.message}
              hint="Tahun angkatan asli siswa di sekolah asal"
              required
            />
          </div>
        </div>
...
function StepKeluarga({ guardianType }: { guardianType: string }) {
  const { register, formState: { errors } } = useFormContext<PindahanFormValues>();
...
// ── Step 4: Alamat Tinggal ──
function StepAlamat() {
  const { register } = useFormContext<PindahanFormValues>();
